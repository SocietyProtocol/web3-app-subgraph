 ...
 *  Otherwise only the auction whose id matches the filter is indexed. */
function isTargetAuction(auctionId: BigInt): boolean {
  return AUCTION_ID_FILTER == "0" || auctionId.toString() == AUCTION_ID_FILTER;
}

export function handleAuctionCleared(event: AuctionCleared): void {
  if (!isTargetAuction(event.params.auctionId)) return;
...
export function handleNewAuction(event: NewAuction): void {
  ...
  let user =
    AUCTION_ID_FILTER != "0"
      ? findOrCreateAuctionUser(userId, event.transaction.from)
      : AuctionUser.load(userId.toString());
  if (!user) {
    return;
  }
